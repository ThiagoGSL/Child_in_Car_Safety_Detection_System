      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // Lista rolável apenas com dispositivos disponíveis
            Expanded(
              child: Obx(() {
                final otherDevices = controller.foundDevices.where((d) =>
                    !controller.isConnected.value ||
                    d.name != controller.connectedDeviceName.value).toList();
            ...
            Obx(() {
              return ElevatedButton.icon(
                onPressed: controller.isScanning.value || controller.isConnecting.value
                    ? null
                    : () {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          'RSSI: ${device.rssi}',
          style: TextStyle(
            color: connected ? Colors.white70 : Colors.black54,
          ),
        ),
        trailing: Obx(() {
          if (connected) {
            ...
            return ElevatedButton(
              onPressed: controller.isConnecting.value || controller.isConnected.value
                  ...
                  : () {
                      ...
                      _showSnackbar(
                        Get.context!,
                        'Conectando ao dispositivo ${device.name}...',
                      );
                    },
